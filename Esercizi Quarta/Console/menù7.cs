...
namespace Primo_progetto
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Contratto[] cancellati = new Contratto[3];
            do
            {
                switch (Scelta(opzioni))
                {
                    ...
                    case 4:
                        if (contatore > 0)
                            CancellaContratto(archivio, cancellati); break;
                    case 5:visualizzaCanc(cancellati); break;
        ...
        static void CancellaContratto(Contratto[] archivio, Contratto[] cancellati)
        {
            int scelta;
            Console.Clear();
            ...
            for (int i = 0; i < archivio.Length; i++)
            {
                cancellati[i].nome = archivio[i].nome;
                cancellati[i].cognome = archivio[i].cognome;
                cancellati[i].nCell = archivio[i].nCell;
                cancellati[i].credito = archivio[i].credito;
                cancellati[i].cAbbonamento = archivio[i].cAbbonamento;
        ...
        static void visualizzaCanc(Contratto[] cancellati)
        {
            for (int i = 0; i < cancellati.Length; i++)
            {
                Console.WriteLine("CONTRATTO N° " + (i + 1));
                Console.WriteLine("Nome: " + cancellati[i].nome);
                Console.WriteLine("Cognome: " + cancellati[i].cognome);
                Console.WriteLine("Numero di cellulare: " + cancellati[i].nCell);
                Console.WriteLine("Credito: " + cancellati[i].credito);
                Console.WriteLine("Tipo di abbonamento: " + cancellati[i].cAbbonamento.ToString());
                Console.WriteLine("\n\n");
            }
            Console.ReadLine();
        }
